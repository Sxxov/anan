...
export const getWSRouter = () => {
	...
	router.ws('/connect', async (ws, req) => {
		...
		let distressItem: DistressItem | null = null;
		const distressNotifiedConnections: PingItem[] = [];
		...
		ws.on('message', (message) => {
			...
			try {
				...
				messageConnectionItem = PingItem.from({
					...JSON.parse(message) as PingItem,
					token: String(token),
				});
			} catch (_: unknown) {
				console.error(_);
			...
			const isNoLongerDistressed = (
				messageConnectionItem.isInDistress === false
			&& connectionItem.isInDistress === true
			);

			const isNowDistressed = (
				messageConnectionItem.isInDistress === true
		...
		function emitDistressSignalList(targetConnection: PingItem, signals?: DistressItem[]) {
			signals ??= Ctx.signals.get(connectionItem) ?? [];

			Ctx.clients.get(targetConnection)?.send(
				String(new DistressSignalListWSResponse(signals!)),
			);
		}
	});
