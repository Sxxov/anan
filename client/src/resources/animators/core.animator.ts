...
export class CoreAnimator {
	...
	private __animatorContainersWrapper: $Object | null = null;
	public visibleAnimations: AnimationObject[] | null = [];
	...
	private lottieInstance: LottiePlayer | undefined;
	...
	public async add(
		animationToBeConstructed: DeepPartial<AnimationObject>,
		...
		let lottieObject: LottieObject | null = null;
		let solidObject: SolidObject | null = null;
		...
		switch (type) {
			...
			case 'lottie':
				if (this.lottieInstance == null) {
					this.lottieInstance = await import('lottie-web') as any;
				}
	...
	protected onWindowResize(): void {
		...
		(this.lottieInstance as any)?.resize?.();
	...
	protected onFrame(frame: number): void {
		...
		let currentAnimationsTotalFrames: number | null = null;
		...
		workingAnimations.fastEach((workingAnimation: AnimationObject) => {
			...
			let localFrame = mBezierUtility.at(
				Math.max(
					Math.min(
						(
							((
	...
	public getTotalFramesBeforeIndex(index: number): number | null {
	...
	public static hide(domElement: HTMLUnknownElement): $Object {
		return $(domElement).addClass('hidden');
	}

	public static unhide(domElement: HTMLUnknownElement): $Object {
		return $(domElement).removeClass('hidden');
	}

	public static activate(domElement: HTMLUnknownElement): $Object {
		return $(domElement).addClass('active');
	}

	public static deactivate(domElement: HTMLUnknownElement): $Object {
