...
function create_if_block(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(container, file$2, 111, 2, 3594);
		},
		m: function mount(target, anchor) {
	...
	dispatch_dev("SvelteRegisterBlock", {
		...
		type: "if",
		source: "(111:1) {#if isArrowEnabled}",
		ctx
	});
...
function create_fragment$2(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(component_1, file$2, 92, 0, 3155);
		},
		l: function claim(nodes) {
			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
		},
...
function instance$2($$self, $$props, $$invalidate) {
	...
	function onMouseMove(event) {
		...
		if (isArrowEnabled) {
			$$invalidate(11, arrowRotationDeg = Math.atan((userX - buttonX) / (userY - buttonY)) * (180 / Math.PI) * (userY > buttonY ? -1 : 1));
		}
...
//# sourceMappingURL=CallRoute-6616307c.js.map
